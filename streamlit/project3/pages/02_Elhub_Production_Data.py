# streamlit/Prosject3/02_Elhub_Production_Data.py
...
df_data_raw = load_mongoDB(MONGO_COLLECTION, MONGO_DATABASE)

# Page configuration
st.title("Elhub Production Data in 2021")
st.write("This page will visualize Elhub production data in 2021. \n"\
         "Data is downloaded from https://api.elhub.no/energy-data/v0/price-areas dataset=PRODUCTION_PER_GROUP_MBA_HOUR into remote MongoDB, \n"
         "This page will load the data form remote MongoDB and visualize it.")
...
with col2:
    ...
    selected_productiongroups = st.pills("Select Production Group",pill_alternativ, default='hydro') #, selection_mode="multi")
    st.session_state['selected_prod_group'] = selected_productiongroups
    ...
    #    df_filtered = df_filtered[df_filtered['productiongroup'].isin(selected_productiongroups)]
        df_filtered = df_filtered[df_filtered['productiongroup']==selected_productiongroups]
    ...
    df_filtered.sort_values(by='starttime', ascending=True, inplace=True)
    st.session_state['plot_elhub_data'] = df_filtered
